    ...
    if (window.JpegCamera) {
        ...
        var take_snapshots = function (count) {
            ...
            if (JpegCamera.canvas_supported()) {
                snapshot.get_canvas(three60App.snap());
            }
            else {
...
var three60App = {
    ...
    snap: function (element) {
        ...
        three60App.uploadSnap(three60App.apiUrl, $(element).data("snapshot", this))
    },
    uploadSnap: function (uploadUrl, snap) {
        var api_url = uploadUrl;
        ...
        var snapshot = snap;
        // var snapshot = $(".item.selected").data("snapshot");
        snapshot.upload({api_url: api_url}).done(three60App.handleDone).fail(three60App.handleFailure);
